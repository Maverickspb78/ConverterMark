...
public class WriterVO {
    ...
    public void writeVO(Properties properties) throws IOException {
        File fileSampleVO = new File(properties.getPathInOborot());
        String var10002 = properties.getFileKMPath();
        File fileKM = new File(var10002 + "\\" + properties.getFileName());
        StringBuilder temp = new StringBuilder(70);
        String[] str = properties.getFileName().split("o");
        StringBuilder path = (new StringBuilder(properties.getPathToWriteFile())).append(str[0]).append("Ввод_в_оборот_").append(this.getDateFileName()).append(".csv");
        FileWriter writer = new FileWriter(String.valueOf(path), false);
        System.out.println(fileKM);
        PrintStream var10000 = System.out;
        String var10001 = properties.getFileKMPath();
        var10000.println("Путь к файлу: " + var10001 + "\nИмя файла: " + properties.getFileName());
        ...
        try {
            ...
            while(scannerKM.hasNextLine()) {
                ...
                if (scannerKM.hasNextLine()) {
                    writer.write(String.valueOf(temp.append("\"").append(line.substring(0, 31).replace("\"", "\"\"")).append("\"").append(",,").append(properties.getDateOfManufacture()).append(",").append(properties.getCodeTNVD()).append(",,,,,").append("\n")));
                    ++count;
                    temp.delete(0, temp.length());
                } else {
                    writer.write(String.valueOf(temp.append("\"").append(line.substring(0, 31).replace("\"", "\"\"")).append("\"").append(",,").append(properties.getDateOfManufacture()).append(",").append(properties.getCodeTNVD()).append(",,,,,")));
                    writer.flush();
                    writer.close();
                    ++count;
